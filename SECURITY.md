...
- **ATC audio is the one relay, and it is allowlisted.** `/api/atc/stream/:mount` relays a LiveATC MP3 stream because the app is served over HTTPS and LiveATC edges answer browsers over plain HTTP (mixed content). The mount must be present in the committed directory (`src/layers/atc/liveatcFeeds.js`, generated offline by `scripts/liveatc-feeds-build.mjs`); anything else is a 404 before any upstream contact. Redirects are followed only within `*.liveatc.net`, the upstream connection is dropped the moment the listener disconnects, at most eight listeners are relayed at once, and nothing is buffered, cached or recorded.
- **Rooms are in-memory and unauthenticated by design.** `POST /api/rooms` mints a six-character code (no I/O/0/1) and a creator token; anyone with the code can join over `/api/rooms/:id/ws`. Nothing is persisted: at most 100 rooms of 25 members, the last 100 chat lines and 20 moments per room, everything gone 30 minutes after the last member leaves. The socket refuses cross-origin upgrades (the `Origin` host must match `Host` or `X-Forwarded-Host`, or be listed in `GEV_ROOMS_ALLOWED_ORIGINS`), caps frames at 16 KB and 40/s, limits chat to 5 messages per 5 s, throttles leader state to 4/s, strips control characters from every string, and renders all text as text (never HTML). Room creation is limited to 12/min per address. To share one room server across several app instances, point the others at it with `VITE_ROOMS_BASE_URL` (or `localStorage.gevRoomsBaseUrl`) and list their origins in that server's `GEV_ROOMS_ALLOWED_ORIGINS`; only those origins receive `Access-Control-Allow-Origin` on `/api/rooms*` and pass the socket's origin check. Do not expose the dev server beyond the tailnet/LAN; a room code is the only secret.
- **Profiles are name + PIN, per-device tokens, one JSON file each.** `POST /api/profiles/login {name, pin}` creates a profile on first use (names trimmed, ≤32 chars, case-insensitively unique; PIN 4-8 digits) and otherwise verifies the PIN against an scrypt hash (N=16384, per-profile salt, constant-time compare). Each sign-in mints a random 256-bit bearer token that is stored only as a SHA-256 hash, expires after 180 days, and can be revoked per device (`/logout`) or for every device (`/devices/revoke-all`). Files live under `GEV_PROFILES_DIR` (default `.data/profiles`, gitignored; `/var/lib/gev/profiles` on the Pi), are written atomically (temp + rename, mode 0600) and never contain a PIN or token in clear. Profiles carry preferences only — style, favourite layers, voice mode, room name, home view, saved places — and `PUT /api/profiles/me` is schema-validated (unknown fields rejected, ≤64 KB, ≤200 places) so no provider key can be stored. Sign-in is limited to 10/min per address; every login, create, logout and revoke writes one log line. Cross-origin calls follow the rooms policy: the `Origin` host must match `Host`/`X-Forwarded-Host` or be listed in `GEV_PROFILES_ALLOWED_ORIGINS` (falls back to `GEV_ROOMS_ALLOWED_ORIGINS`). In the browser only the token (localStorage, or sessionStorage when "remember" is off) and a copy of the profile are kept; the PIN is never written anywhere, and every user string is rendered with `textContent`.
