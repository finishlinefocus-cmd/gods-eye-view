...
- **Rooms are in-memory and unauthenticated by design.** `POST /api/rooms` mints a six-character code (no I/O/0/1) and a creator token; anyone with the code can join over `/api/rooms/:id/ws`. Nothing is persisted: at most 100 rooms of 25 members, the last 100 chat lines and 20 moments per room, everything gone 30 minutes after the last member leaves. The socket refuses cross-origin upgrades (the `Origin` host must match `Host` or `X-Forwarded-Host`, or be listed in `GEV_ROOMS_ALLOWED_ORIGINS`), caps frames at 16 KB and 40/s, limits chat to 5 messages per 5 s, throttles leader state to 4/s, strips control characters from every string, and renders all text as text (never HTML). Room creation is limited to 12/min per address. Do not expose the dev server beyond the tailnet/LAN; a room code is the only secret.
